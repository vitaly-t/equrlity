...
export function calcHashBuffer(buf: Buffer): string {
  const hasher = create();
  hasher.update(buf)
  return hasher.digest('hex');
}

export async function calcHash(f: any /* ReadStream */): Promise<string> {
  const hasher = create();
  f.on('data', buf => {
    hasher.update(buf)
  });
  await new Promise(resolve => { f.on('end', resolve); });
  return hasher.digest('hex');
}
